...
1. 組 prompt：經文＋全部來源全文＋本章新建條目清單＋份量要求＋wiki-link 白名單（A/B 既有條目＋本章實建 C 條目）。
...
| `raw_data/{commentary}_{book}_{ch}.txt` | `crawl_bible_text.py` | source_excerpts → M3/M6 prompt 全文餵入 |
| `raw_data/stepbible_{book}_{ch}.txt` | `extract_stepbible.py` | 同一 source_excerpts loader → M3/M6 原文證據 |
