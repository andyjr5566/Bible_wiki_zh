...
flowchart TD
    ...
    subgraph S1["步驟1：準備來源（Agent）"]
        ...
        STEP["extract_stepbible.py<br>擷取 STEP 原文資料"]
...
四套 commentary（URL 模式見 [[scheme]] §7）用 `crawl_bible_text.py`：
...
STEP 原文證據層另走固定 extractor，不抓 stepbible.org HTML：

```text
python util/extract_stepbible.py "【書名】 X" --data_path .stepbible_data --output_path raw_data --download
```

`.stepbible_data/` 是可重建且不入庫的官方資料集快取；正式章檔是 `raw_data/stepbible_{english_book}_{chapter}.txt`。STEP 可支持詞形、lemma、Strong、morphology、context gloss 與 lexicon 義域，但不是第五套 commentary、不計入 commentary 共識；lexicon 義域與 morphology 不得越界當成本節語境義或神學結論。

正式來源與 prompt context 是兩層：`raw_data`／manifest 永遠保留完整 source of truth；四套 prose commentary 由 Agent 全文閱讀並以 `read_log.md` 驗後段引句，STEP 完整檔則由 deterministic parser 驗證並寫 `step_source_receipt.json`。Manual M3/M6 不再複製已讀 commentary 全文，只列 canonical `類型（identity）` path/URL；STEP 使用 compact projection，需要深查時執行 `python util/step_context.py 書名 章 --verses ...|--strong ...|--word ...`。

### 2.2.1 已完成章的 STEP Phase A 回填

創世記 1–50 與出埃及記 1–40 已完成 STEP-only 基礎設施回填，共 90 章：每章有正式 STEP raw、manifest 的 STEP 第五列與 deterministic machine receipt。這只是補齊來源層，兩卷仍維持「已完成／已勘誤」狀態；沒有重跑 M3、M6、render、links、commentary crawler、commentary read receipt 或重新校讀。

```text
python util/backfill_step_sources.py "創世記" --dry-run
python util/backfill_step_sources.py "創世記"
python util/backfill_step_sources.py "出埃及記"
python util/audit_original_language.py "創世記"
python util/audit_original_language.py "出埃及記"
```

`backfill_step_sources.py` 支援 `--chapters 1,3,5-8`、`--dry-run` 與明示的 `--force`，可重跑且既有有效檔預設只驗證／沿用；損壞檔會 fail closed，不會靜默覆寫。`audit_original_language.py` 只讀章檔與本章相關／歸屬的 link entries，輸出 `reports/step_audit/*_findings.json` 與 `*_summary.md`，分類為 PASS、REVIEW_LOW／MEDIUM／HIGH、UNVERIFIABLE、INTERPRETIVE，絕不自動修 production content。利未記／民數記不要預先批次回填，等各章實際進入正式校讀時才整合 STEP。

### 2.3 程式產生 `source_manifest.md`

先檢查每份 raw text 是否為**本章**有效內容（404／目錄頁／亂碼／無關／STEP 零匹配＝無效），再執行 `python util/build_source_manifest.py 【書名】 X`。不可手寫；程式依磁碟現況產生四套註釋＋STEP 五列，表格欄位固定：`來源｜類型｜URL｜raw_data 檔案｜狀態`。註釋缺檔提示 crawler，STEP 缺檔提示 extractor。

這張表之後是**唯一事實來源**：
- 只有狀態 OK 的來源會進 provenance／validation；manual prompt context 另依 source kind 投影。
- 條目「來源依據」與章節「參考資料」的 URL 都由同一 canonical identity/type mapping 取用／驗證：`逐節註解（ccbiblestudy CT）`、`拾穗（ccbiblestudy GT）`、`研經註解（KingComments）`、`研經註解（BibleHub Study）`、`原文資料（STEP Bible）`。舊 payload 的 CT/GT/KC/BH/STEP 與已實存的 bare `研經註解` 經稽核後相容（後者只在同一行有精確 manifest URL 時靠 URL 決定 identity）；新 prompt 一律 canonical，不讓模型猜 label。
...
2. `_batch_entry_prompt()` 仍只有一份 schema。Injected/default runner 維持 `FULL` context；manual runner 改用 `MANUAL_PROJECTED`：commentary 只列已依 `sources.md` 全讀的 path/URL，不再內嵌 raw body；STEP 從該 batch 的 `evidence` 與 `surfaces[].verses` 取節次，產生 occurrence＋exact Extended Strong 去重 lexicon。自然語言 evidence 無法解析時 fail-open 到本章 compact STEP，不會靜默省略。
...
1. 組 prompt：經文＋source context＋本章新建條目清單＋份量要求＋wiki-link 白名單（A/B 既有條目＋本章實建 C 條目）。Manual context 不重貼 commentary，STEP 採章級 compact projection（每個 occurrence 保留原文、音譯、exact Extended Strong、morphology code、gloss；brief lexicon 依 exact Extended Strong 只列一次）。
2. `_org_requirements()` 依節數算門檻（≥15 節要 3 個 `### 小節（vX-Y）`、字數 `max(400, min(1500, 節數×40))`），`_chapter_payload_validator()` 強制執行——防止模型交三行條列。門檻只管份量不管體裁，prompt 分「硬規格／設計空間／禁用」三層：硬規格之內模型自由設計呈現（參創8 的散文混表格）。
3. 模型填 `knowledge_nodes`（分組→節點清單）＋ `organization`（整合性敘述，標明「CT指出…」等出處）。Obsidian 設計空間採 Visual Thinking：先判斷資訊的形狀再選形式，不預設全寫散文——理解與整合→散文（主幹）、比較對照→表格、流程因果→flowchart、時間發展→timeline、分類階層→mindmap、關係空間→graph、對答往返→sequenceDiagram、補充旁註→callout。行文首次提及本章條目時以 `[[完整條目名|行文用詞]]` 連結——目標限白名單（壞連結擋下）、且至少一個（出34 零連結的教訓）；callout 各型自由用（`[!quote]` 引句、`[!note]`/`[!info]` 標「來源解讀非經文明言」、`[!important]` 樞紐、`[!question]` 懸題、`> [!example]-` 可摺疊收補充）；`==高亮==` 可用；mermaid 限 flowchart／graph／timeline／mindmap／sequenceDiagram，節點標籤雙引號防破圖，圖旁配散文。欄杆：散文主幹 ≥ 字數門檻一半（`_org_prose_chars()`，fence 內文不計，擋大半內容包進表格/callout/圖表）；`![[]]` 嵌入與 mermaid 以外的程式碼區塊硬禁；mermaid 圖型超出清單、fence 未閉合、圖內出現 `[[`（會被 verify_links 掃成 BROKEN）都退回（`_org_split_fences()`＋`_mermaid_type()`；fence 先拆再驗 wiki-link 與散文計數，防 `A[[x]]` 節點語法誤判）。
...
```text
02 出埃及記/.tmp/第26章/
├── source_manifest.md      ← build_source_manifest.py（步驟1，不手寫）
...
| `python util/verify_links.py 【書名】` | 離線掃描 wiki-link（模擬 Obsidian 解析：確切檔名、不認 alias，逐條與 find-unresolved-link 一致），不需開 Obsidian；解決舊版 alias 誤放行（如「按手」）的漏網之魚 | BROKEN=0, INVALID=0, UNKNOWN=0（PENDING 可存在） |
...
| `python -m unittest discover -s util/tests` | 全部工具測試（含 render round-trip 與 STEP OT/NT 整合） | 全過（CI 必跑） |
| `python util/benchmark_step_context.py` | 用本地官方 STEP cache＋真實 Genesis 1 commentary／John 1:1-5，在 temp workspace 量 before/after chars/UTF-8 bytes | M3/M6 prompt reduction 均須 ≥30%，不改 production chapter/wiki |
| `python util/backfill_step_sources.py 【書名】 --dry-run` | 已完成章的 STEP-only migration 預演；可加 `--chapters` | 不改 production，報告 generated/reused/failed 與 safety hashes |
| `python util/audit_original_language.py 【書名】` | 以本章 STEP 稽核既有原文／音譯／Strong／morphology 陳述 | 只寫 JSON＋Markdown 報告，不改章檔或 link entries；人工處理 review queue |
...
| `raw_data/{commentary}_{book}_{ch}.txt` | `crawl_bible_text.py` | 完整 provenance＋Agent 全文閱讀/read_log；manual M3/M6 只引用 path/URL |
| `raw_data/stepbible_{book}_{ch}.txt` | `extract_stepbible.py` | 完整 machine validation/receipt；M3 task verse projection、M6 chapter compact projection、按需 query |
| `.tmp/第x章/source_manifest.md` | `build_source_manifest.py` | source_excerpts（所有 OK 正式來源篩選、URL 唯一事實來源） |
