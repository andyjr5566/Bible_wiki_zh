...
3. 模型填 `knowledge_nodes`（分組→節點清單）＋ `organization`（整合性敘述，標明「CT指出…」等出處）。Obsidian 設計空間：行文首次提及本章條目時以 `[[完整條目名|行文用詞]]` 連結——目標限白名單（壞連結擋下）、且至少一個（出34 零連結的教訓）；callout 各型自由用（`[!quote]` 引句、`[!note]`/`[!info]` 標「來源解讀非經文明言」、`[!important]` 樞紐、`[!question]` 懸題、`> [!example]-` 可摺疊收補充）；`==高亮==` 可用；材料是流程／關係／結構時歡迎 mermaid 圖表（限 flowchart／graph／timeline／mindmap／sequenceDiagram，節點標籤雙引號防破圖，圖旁配散文）。欄杆：散文主幹 ≥ 字數門檻一半（`_org_prose_chars()`，fence 內文不計，擋大半內容包進表格/callout/圖表）；`![[]]` 嵌入與 mermaid 以外的程式碼區塊硬禁；mermaid 圖型超出清單、fence 未閉合、圖內出現 `[[`（會被 verify_links 掃成 BROKEN）都退回（`_org_split_fences()`＋`_mermaid_type()`；fence 先拆再驗 wiki-link 與散文計數，防 `A[[x]]` 節點語法誤判）。
