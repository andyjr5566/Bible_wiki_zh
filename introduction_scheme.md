...
書卷導論一旦存在，就成為該書卷在首頁的**主要入口頁**。標準閱讀鏈為：
...
- 根目錄 `index.md`：若該卷已有書卷導論，書卷名稱直接連到完整路徑的 `書卷導論`；尚未建立導論的書卷可以暫時維持原本的目錄入口。
- `{書卷}/書卷導論.md`：頂部與底部導航至少提供完整路徑的 `全書目錄及綱要` 與 `第1章`。
- `{書卷}/全書目錄及綱要.md`：提供完整路徑的 `書卷導論` 與 `第1章`；章節清單亦應使用 `{書卷資料夾}/第N章` 完整路徑。
- renderer 預設輸出必須是 `{book_folder}/書卷導論.md`，不得重新生成英文 `Introduction.md`。
- checker 與 deploy 前 review 應把上述路徑契約視為書卷導論 rollout 的必要 gate。

這個規則不是為了 Obsidian 本機解析方便，而是為了讓 repo、Quartz／網站 deploy 與未來自動生成流程對同一個目標檔案有唯一、穩定的識別方式。
...
只負責給讀者閱讀，不承擔 research log。它必須由 renderer deterministic 產生，並作為該書卷的公開入口頁。
