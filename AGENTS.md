...
- **`link_folder/` 條目的 `## 定義` 與 `## 主題發展` 裡，經文引註一律寫成「書卷簡稱＋章:節」（`申32:15`、`民35:33`、`出19:4`），不得只寫「第15節」。** 條目是跨書卷跨章的活文件，讀者從任何一章連過來都可能落在這兩個區塊，裸節號無法判斷是哪一卷哪一章。章節主檔 `第x章.md` 的「本章整理」與條目 `## 按書卷累積` 底下的逐章區塊有章號標題涵蓋，可以續用「第15節」。
