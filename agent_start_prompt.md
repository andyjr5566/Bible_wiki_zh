...
5b. **引句逐字回查（機械，先跑再人工複核）**
   ```text
   python util/check_quote_fidelity.py 【書名】 X
   ```
   把本章 `entry_content`、`chapter_content`、`link_updates` 的 summary／relation，
   以及渲染後的「本章整理」裡所有「」『』引句，逐條回查本章 manifest 的 OK 註釋＋
   **全卷**經文（引用前面章節的經文是正常寫法；引用相鄰章的**註釋**才是要抓的那一型）。
   比對前把引號、空白、全半形正規化掉，帶 ⋯ 的節錄逐段查。
   這一關驗的是其他閘門驗不到的東西——引號裡那句話出自哪裡。實測最常見的四種飄移：
   截斷後自己補句號、換一種引號、把英文來源的中文譯文當成逐字引句、把不相鄰的兩句
   接成一句。它已接進步驟8 的 `check_chapter_files` 步驟5，但那時才發現要重跑 render，
   所以寫完 payload 就先跑一次。**報出＝強力線索；未報出不代表內容忠實**（掛錯家、
   壓平爭議、腦補引註仍要人工讀）。

   **預設門檻是 10 個字，比它短的引句一個字都不比對**，所以「」被當成強調記號或術語
   標記用（本檔明文禁止的一型）時，這一關全綠也抓不到。**送 reviewer 之前**再跑一次
   降門檻版：
   ```text
   python util/check_quote_fidelity.py 【書名】 X --min-chars 2
   ```
   誤報會變多（來源真的只給兩三個字的字義是正當引句），所以**報出的每一條都要人工
   裁決，不可照著刪**。要在 submit 前跑：payload 一旦通過 review 才修，就得重新 submit，
   兩次 reviewer 預算會被這種標點問題吃掉（申25 實錯）。條目模式同樣吃這個參數。
...
- **英文來源（KingComments、BibleHub）一定要讓中文讀者讀得懂，但「」裡只能放逐字原文。** 兩種寫法，選一種：
  ① `KC：「An iron furnace is fired as hot as possible in order to process the metal afterwards.」（鐵爐要燒到最熱，才能接著加工金屬。）`——英文原句進「」，中譯緊接在（）內；
  ② 具名轉述、**不加引號**：`KC 把兩種偏差配上兩種人——法利賽人把自己的誡命加在律法上，撒都該人把心智推不通的部分從律法裡拿掉。` 轉述要把那句話的內容講完，不可縮成「KC 也談到這點」的摘要體。
  **不可以把中譯放進「」**。那等於宣稱這是逐字原話，而來源根本不是中文；`check_quote_fidelity` 必然報出來（申1／申2 實測 10 處，全是這一型）。整段貼英文原文同樣不行——本檔開頭已訂「所有輸出用繁體中文」，①的中譯就是為此而設。
- **「」是逐字宣告，不是強調記號，也不是術語標記。** 引號裡的字要能原樣回查到本章來源或和合本經文，所以：
  - 截斷來源時句號放在引號**外**（`「…相接的丘陵地帶」。`）——來源那句話還沒結束，`「…丘陵地帶。」`是替來源多加了一個它沒有的句點（申1／申2 實測 25 處）。
  - 不相鄰的兩句要用 `⋯` 接（`「按神的計畫…剛硬⋯故此，人不可忽視自己的行動…」`），不可直接黏成一句。
  - **STEP 逐詞的中文字面直譯不加引號**：`字面是沒有一座城對我們太高`、`mi.Le' 'a.cha.Rei Yah.weh 直譯是填滿了跟在耶和華後面`。那是你的翻譯，不是誰的原話。
  - 自己對經文結構的概括也不加引號（`記的都是同一件事：先前住在那裡的人被後來的人取代`）。
