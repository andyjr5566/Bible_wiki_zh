...
STEP_USAGE_RULES = (
    ...
    "註釋來源支持。\n"
    "- 表達不同層次時要正面分層：先說 STEP 能確認的字形、lemma、Strong、morphology、"
    "本節譯義或簡要義域，再說部分註釋如何從整段或其他經文進一步理解。除非原文、"
    "Strong、詞形或 lexical identification 確有可驗證衝突，否則不要寫成『這個字不能"
    "證明……』『STEP 沒有這個意思』『這只是神學推論』等反駁或裁決語氣。\n"
...
def _batch_entry_prompt(ctx, batch, allowed_related, sources_text, raw_text,
    ...
    return (
        ...
        f"- **definition 與 development 裡的經文引註一律寫「書卷簡稱＋章:節」"
        f"（申32:15、民35:33、出19:4），不可只寫「第15節」。** 條目是跨書卷跨章的活文件，"
        f"讀者從任何一章連過來都可能落在這兩個區塊，裸節號無從判斷是哪一卷哪一章。"
        f"accumulations 的 summary／relation 在條目裡有章號標題涵蓋，可以續用「第15節」。\n"
        f"- status 一律 formal。accumulations 是「物件陣列」，每項含 book、chapter、"
        f"summary、relation 四欄，且至少含本章（{ctx.book} 第{ctx.chapter}章）一筆、同章只給一筆。\n"
        f"- development（選填）只收「跨書卷／跨章的主題遞進或預表發展」；同一段經文內的字義、"
        f"詞源、各家詮釋辯論寫進 definition，不要塞進 development；也不要逐句重述某一筆 "
        f"accumulations 已有的 summary／relation——沒有真正跨卷的內容就留空，不要硬湊。"
        f"若此條目是既有條目（已有其他章節的 accumulations），development 必須反映目前全部"
        f"已累積的範圍，不能只停留在最早那一章／那一卷。\n"
...
def chapter_content_step(
    ...
    prompt = (
        ...
        f"直接用純文字提及即可，不要加 [[ ]]。\n"
        f"- 所有輸出用繁體中文，但「」是逐字宣告而不是強調記號：引用英文來源"
        f"（KingComments、BibleHub）時只有兩種寫法——①英文原句放進「」，"
        f"緊接著在（）內附上繁體中文翻譯；②具名轉述並完全不加引號，"
        f"且要把那句話的內容講完，不可縮成「KC 也談到這點」的摘要體。"
        f"**不可把中譯放進「」**：來源說的不是中文，那等於偽造逐字引句，"
        f"check_quote_fidelity 會逐條回查並報出。也不可整段貼英文原文。"
        f"只有原文用字本身是重點時，才以括號附註原文詞。\n"
