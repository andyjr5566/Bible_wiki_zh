    ...
    try:
        ...
        result.update({
            ...
            "next_step": (
                ...
                "只有 preview 提出 challenge 而仍 keep 時才填受控 basis（already_covered 另須 covered_by 逐字錨點）；"
...
def find_step_candidates(
    ...
    try:
        ...
        return {
            ...
            "max_results": max_count,
            "truncated": truncated,
            "chapters_scanned": diagnostics.get("scanned_chapters", []),
            "missing_chapters": diagnostics.get("missing_chapters", []),
            "invalid_chapters": diagnostics.get("invalid_chapters", []),
        }
...
7. B 類累積先用 `prepare_chapter_link_updates`。先完整讀取 `link_updates.yaml` 每筆 `path` 指向條目的定義、全部按書卷累積與主題發展，再逐條填 `overview_review`：`definition` 判斷穩定身分／辨識邊界是否改變，`development` 判斷是否形成至少跨兩章的推進／轉折／對照；兩者只填 `keep` 或 `update`。這是審查義務、不是更新配額；`keep` 是正常完整的結果，全部 `keep` 也可通過。只有 preview 提出程式 challenge 而仍選 `keep` 時，才依錯誤訊息填一個受控 `basis`；`update` 由正式區塊 diff 證明。逐章累積的 `summary`／`relation` 只記本章事實與關聯，絕不可換句話說後塞進定義或主題發展；`development=update` 須列 `synthesis_scope`（本章＋至少另一章）並先修改條目。再核對來源、呼叫 `preview_chapter_link_updates` 查看區塊 diff，使用回傳 token 才可 `apply_chapter_link_updates`；套用後重跑 preview 必須是 0 變更。
