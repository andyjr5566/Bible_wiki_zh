...
路徑規則：所有檔案系統路徑一律使用 Windows 格式，例如 `C:\Obsidian\Hermes\scripture\raw_data\xxx.txt`。Obsidian wiki-link 內部目標仍使用 Obsidian 慣例 `/`，例如 `[[但以理書/第3章|但以理書 第3章]]`。
...
└── link_folder\
    ├── 人物\ 地點\ 主題\ 背景\ 歷史\ 原文\ 文化\ 神學\ 互文\ 解經爭議\
    ├── _待分類\
...
```text
python crawl_bible_text.py "{URL}" --output_path "C:\Obsidian\Hermes\scripture\raw_data" --output_filename "{source}_{book_slug}_{chapter}"
```

例：

```text
python crawl_bible_text.py "https://biblehub.com/study/daniel/3.htm" --output_path "C:\Obsidian\Hermes\scripture\raw_data" --output_filename "biblehub_study_daniel_3"
```

結果：`C:\Obsidian\Hermes\scripture\raw_data\biblehub_study_daniel_3.txt`。檔名不用加 `.txt`；已存在時不覆蓋，只有確認舊檔錯誤或使用者要求時才加 `--overwrite`。

建議命名：

```text
C:\Obsidian\Hermes\scripture\raw_data\biblehub_study_daniel_3.txt
C:\Obsidian\Hermes\scripture\raw_data\kingcomments_daniel_3.txt
C:\Obsidian\Hermes\scripture\raw_data\ccbiblestudy_CT_daniel_3.txt
C:\Obsidian\Hermes\scripture\raw_data\ccbiblestudy_GT_daniel_3.txt
```
...
例：27Daniel/27CT03.htm, 27Daniel/27GT03.htm
