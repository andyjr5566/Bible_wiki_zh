...
**進度（`chore/pipeline-backlog` 分支）**：A1–A4、B1–B6、C1–C4 全數完成（B7 為 doc-only，本檔已列為此類）。
每項在下方各自的「狀態」段記錄實作內容與測試；`util/tests` 全 discover 561 tests，
既有 2 項 `test_link_workflow` 失敗與本次無關（開工前 `920640b5` 即存在，屬 `review_evidence.md` 段落索引截斷）。
未動：A4 附帶的「reviewer 對 util/ 硬性 read-only」屬 sandbox／權限設定，非本檔程式碼範圍。
...
4. 其餘依卷別排程；C 級可在任一次工程回合順手做掉。✅（B3、B5、B6、B7、C1–C4 均已完成）
...
| --- | --- |
| `作廢過期下游改為搬到 .trash/` | A1 |
| `--observed-sha 補記 + stage hash 涵蓋清單` | A3、A4 |
| `重 render 保留附錄資源區塊 + 一致性閘門` | B2 |
| `統一 write_yaml_atomic` | A2 |
| `M3 gate PASS 後凍結 link_plan 分桶` | B1 |
| `GT 子來源護欄辨識裸名出處標記` | B4 |
| `forced_pass 卷末統計 + Attempt 1 checklist + 舉證格式` | B6、B7 |
| `短偽引句裁決紀錄閘門 + 主題發展形狀規則進 prompt` | B3、B5 |
| `正向孤兒全庫掃描 + 章 md 路徑連結檢查` | C2、C4 |
| `候選存在性掃描紀錄閘門 + util 腳本自帶 UTF-8 保險絲` | C1、C3 |

新增／修改的 `check_chapter_files` 閘門（都只在對真正庫根、對應前置檔存在時才判）：
候選存在性掃描紀錄、短偽引句裁決紀錄、附錄資源區塊、章 md 的 markdown 路徑連結。
