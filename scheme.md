...
H2 順序固定為上列順序。必填 H2：`定義`、`按書卷累積`、`來源依據`。選填 H2：`主題發展`、`相關條目`——可以缺少，但若存在則必須在正確位置。`定義` 同時承擔條目辨識與核心摘要，不另設內容容易重複的「核心摘要」區。Agent 只能在以下累積區新增：`按書卷累積`、`相關條目`、`來源依據`。除非使用者明確要求，不得大改 `定義`、`主題發展`。
...
- `link_updates.py` 定位累積區時，依序嘗試 `## 主題發展` → `## 相關條目` → `## 來源依據` → 檔尾作為結束邊界；既有條目缺少 `## 主題發展` 不影響套用。
...
`util/validate_knowledge_base.py` 檢查 YAML、分類、alias、重複累積標記、合法同名詞、歧義裸 WikiLink、正式條目結構與保護區。既有技術債可列 warning；本次新增或更新檔案若違規則為 blocking。H2 順序檢查：必填 H2（`定義`、`按書卷累積`、`來源依據`）缺少為 error；選填 H2（`主題發展`、`相關條目`）可缺少，但存在的 H2 必須按 scheme 順序排列。
