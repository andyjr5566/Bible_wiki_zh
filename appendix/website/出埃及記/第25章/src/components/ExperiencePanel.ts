...
function renderLearning(state: Readonly<ExperienceState>): string {
  ...
  const rituals = learning.objectId === 'burnt-altar' ? '' : learning.ritualIds.map((id) => `<button type="button" class="primary-button" data-ritual-id="${escapeHtml(id)}">播放儀式：${escapeHtml(id)}</button>`).join('');
