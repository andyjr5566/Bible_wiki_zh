...
- status 一律 formal。accumulations 是「物件陣列」，每項含 book、chapter、summary、relation 四欄，且至少含本章（民數記 第11章）一筆、同章只給一筆。
- related_entries 只能從此清單選（用完整條目名，不可用「創3:24」這類裸經文引用）：閒雜人, 閒雜人是誰（出12：38）, 嗎哪, 嗎哪的物理特性, 百姓厭棄嗎哪（約6：31-35）, 以色列的長老, 七十長老制度起源, 聖靈, 聖靈分賜預表五旬節（徒2：1-4）, 約書亞, 鵪鶉遷徙, 鵪鶉遮營, 哈洗錄, 他備拉, 摩西的靈命低谷與重擔求死, 伊利達與米達, 基博羅哈他瓦。
- sources 每項格式「標籤: 位置說明（URL）」，標籤與 URL 必須成對取自本章來源：逐節註解: https://www.ccbiblestudy.org/Old%20Testament/04Num/04CT11.htm；拾穗: https://www.ccbiblestudy.org/Old%20Testament/04Num/04GT11.htm；研經註解: https://www.kingcomments.com/en/bible-studies/Num/11；研經註解: https://biblehub.com/study/numbers/11.htm——標籤寫錯對應（如 KC 標籤配 CT 的 URL）視為錯誤。
