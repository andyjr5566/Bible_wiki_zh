...
- 這是 Obsidian 筆記庫：行文首次提到本章條目時用 wiki-link 連結，用詞不同寫 [[完整條目名|行文用詞]]，之後再提不必重連；至少要有一個連結，且目標只能取自本章可連條目：摩西、亞倫、燔祭、贖罪祭、贖愆祭（asham）、平安祭、素祭（minchah）、搖祭（tenufah）、舉祭（terumah）、不可挨近死屍（拿細耳人的潔淨）、亞倫的祝福（祭司祝福）、剃頭刀不可過頭（拿細耳人長髮）、平安（shalom）、意外沾染死屍與重新潔淨條例、拿細耳人滿期還俗條例、拿細耳人（nazir）、臉光照（神的容面）、遠離葡萄樹與酒、離俗歸耶和華——不可連清單外的目標；清單裡沒有對應條目的概念（vault 裡存在與否都一樣），直接用純文字提及即可，不要加 [[ ]]。
