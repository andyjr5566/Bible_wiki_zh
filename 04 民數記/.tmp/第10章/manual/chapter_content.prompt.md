...
- 這是 Obsidian 筆記庫：行文首次提到本章條目時用 wiki-link 連結，用詞不同寫 [[完整條目名|行文用詞]]，之後再提不必重連；至少要有一個連結，且目標只能取自本章可連條目：銀號、拔營起行、巴蘭的曠野、何巴、約櫃前行與停住的宣告、會幕（帳幕整體）、燔祭、平安祭（shelamim）、法櫃的帳幕、雲彩（anan）、西乃的曠野、革順子孫、米拉利子孫、哥轄子孫、流珥葉忒羅與何巴的稱謂關係、約櫃——不可連清單外的目標；清單裡沒有對應條目的概念（vault 裡存在與否都一樣），直接用純文字提及即可，不要加 [[ ]]。
