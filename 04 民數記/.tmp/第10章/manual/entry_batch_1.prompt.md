你是聖經研經資料整理員，並且輸出以繁體中文為主。任務：一次為以下 5 個 link_folder 條目各填一份 entry_content payload。
...
- status 一律 formal。accumulations 是「物件陣列」，每項含 book、chapter、summary、relation 四欄，且至少含本章（民數記 第10章）一筆、同章只給一筆。
- related_entries 只能從此清單選（用完整條目名，不可用「創3:24」這類裸經文引用）：銀號, 拔營起行, 巴蘭的曠野, 何巴, 約櫃前行與停住的宣告。
- sources 每項格式「標籤: 位置說明（URL）」，標籤與 URL 必須成對取自本章來源：逐節註解: https://www.ccbiblestudy.org/Old%20Testament/04Num/04CT10.htm；拾穗: https://www.ccbiblestudy.org/Old%20Testament/04Num/04GT10.htm；研經註解: https://www.kingcomments.com/en/bible-studies/Num/10；研經註解: https://biblehub.com/study/numbers/10.htm——標籤寫錯對應（如 KC 標籤配 CT 的 URL）視為錯誤。
