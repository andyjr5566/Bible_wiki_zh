...
- 這是 Obsidian 筆記庫：行文首次提到本章條目時用 wiki-link 連結，用詞不同寫 [[完整條目名|行文用詞]]，之後再提不必重連；至少要有一個連結，且目標只能取自本章可連條目：米利暗卒於加低斯、摩西擊打磐石兩下、在米利巴未將耶和華尊為聖、以東王拒絕以色列人假道、何珥山、亞倫卒於何珥山與祭司職任更替——不可連清單外的目標；清單裡沒有對應條目的概念（vault 裡存在與否都一樣），直接用純文字提及即可，不要加 [[ ]]。
