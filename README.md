...
詳見 [`scheme.md`](./scheme.md) **第二十三節「章節處理流程」**
...
3. 建立 link_index
   ↓
4. 抽取 link_candidates
   ↓
5. 比對 → 產生 link_plan
   ↓
6. 寫入章節主檔 + link folder 條目
   ↓
7. verify_links.py → 0 broken links
   ↓
8. link_quality_check.py → 0 critical
   ↓
9. git commit + push
```
...
1. **已收集資料明確提到** — 不猜測、不臆測
2. **有足夠內容支撐** — 不為空 stub
3. **有對應本地檔案** — 必須閉合（不能懸空 link）

### ❌ 禁止的做法

| 禁止 | 原因 |
|------|------|
| `[[何西阿]]書` | 應為 `[[何西阿書]]`，區分人物與書卷 |
| `[[弓]]` | 普通名詞，除非來源解釋其象徵意義 |
| 同名條目多處建立 | 應只有一個主條目 + secondary_types |
| 跳過 verify | 會產生破損連結 |
| 為未來章節預建空檔案 | 污染進度 |

詳見 [`scheme.md`](./scheme.md) **第二十五節「禁止事項」**
...
1. ✅ 完成中級步驟
2. ✅ 深入學習 [`scheme.md`](./scheme.md) **二十二節「link 治理」**
3. ✅ 掌握 Python 驗證工具
4. ✅ 管理全域 `link_folder/`
5. ✅ 維護 `_config/bible_books.json`
6. ✅ 主導新書卷規劃
